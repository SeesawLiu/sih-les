    ...
    public class WebAppBaseController : BaseController
    {
            ...
            get
            {
                if (!_maxRowSize.HasValue)
                {
                    _maxRowSize = int.Parse(this.genericMgr.FindById<com.Sconit.Entity.SYS.EntityPreference>
                        (com.Sconit.Entity.SYS.EntityPreference.CodeEnum.MaxRowSizeOnPage).Value
                        );
                }
        ...
        private IList<T> ExcutePageDataHql<T>(SearchStatementModel searchStatementModel, GridCommand command)
        {
            ...
            if (command != null)
            {
                ...
                result = genericMgr.FindAll<T>(searchStatementModel.GetSearchStatement(), searchStatementModel.Parameters, (command.Page - 1) * command.PageSize, command.PageSize);
            }
            else
            {
                result = genericMgr.FindAll<T>(searchStatementModel.GetSearchStatement(), searchStatementModel.Parameters);
            }
