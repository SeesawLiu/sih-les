...
{
    public partial class UCUnFreeze : UCBase
    {
        public new event MainForm.ModuleSelectHandler ModuleSelectionEvent;

        private static UCUnFreeze ucUnFreeze;
        private static object obj = new object();

        private UCUnFreeze(User user)
            : base(user)
        {
            this.InitializeComponent();
        }

        public static UCUnFreeze GetUCUnFreeze(User user)
        {
            if (ucUnFreeze == null)
            {
                lock (obj)
                {
                    if (ucUnFreeze == null)
                    {
                        ucUnFreeze = new UCUnFreeze(user);
                    }
                }
            }
            ucUnFreeze.Reset();
            ucUnFreeze.lblMessage.Text = "请扫描需要解冻的条码";
            return ucUnFreeze;
        }
        ...
        protected override void ScanBarCode()
        {
            ...
            if (!this.isCancel)
            {
                if (op == CodeMaster.BarCodeType.HU.ToString())
                {
                    ...
                    else if (hu.IsFreeze == false)
                    {
                        throw new BusinessException("条码{0}未被冻结", hu.HuId);
                    }
        ...
        protected override void DoSubmit()
        {
            try
            {
                ...
                this.smartDeviceService.InventoryUnFreeze(this.hus.Select(h => h.HuId).ToArray(), this.user.Code);
                this.Reset();
                this.lblMessage.Text = "解冻成功";
            }
            catch (Exception ex)
            {
                this.isMark = true;
