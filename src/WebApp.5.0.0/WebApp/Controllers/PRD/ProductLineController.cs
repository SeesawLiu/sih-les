...
namespace com.Sconit.Web.Controllers.PRD
{
    ...
    public class ProductLineController : WebAppBaseController
    {
        ...
        public ActionResult _FeedQtyDetailList(string flow)
        {
            ...
            #region 选默认库位的
            string flowcheckStr = SecurityHelper.CheckFlowStatement(flow, com.Sconit.CodeMaster.OrderType.Production);
            FlowMaster flowMaster = base.genericMgr.FindAll<FlowMaster>(flowcheckStr).SingleOrDefault<FlowMaster>();
            if (flowMaster != null)
            {
        ...
        public ActionResult _BackFlushDetailList(string productLine, string productLineFacility)
        {
            ...
            string flowcheckStr = SecurityHelper.CheckFlowStatement(productLine, com.Sconit.CodeMaster.OrderType.Production);
            FlowMaster flowMaster = base.genericMgr.FindAll<FlowMaster>(flowcheckStr).SingleOrDefault<FlowMaster>();
            if (flowMaster == null)
            {
            ...
            else
            {
                ...
                productLineLocationDetailList = base.genericMgr.FindAll<ProductLineLocationDetail>(hql, para.ToArray());
                ...
                if (genericMgr != null && q.Count() > 0)
                {
                    ...
                    {
                        Item item = base.genericMgr.FindById<Item>(pLLDetail.Item);
                        if (item != null)
                        {
             ...
             "updated")]IEnumerable<BomDetail> updatedBomDetails)
        {
            try
            {
                ...
                string flowcheckStr = SecurityHelper.CheckFlowStatement(productLineLocationDetail.ProductLine, com.Sconit.CodeMaster.OrderType.Production);
                FlowMaster flowMaster = base.genericMgr.FindAll<FlowMaster>(flowcheckStr).SingleOrDefault<FlowMaster>();
                if (flowMaster == null)
                {
                    throw new BusinessException("生产线不正确或没有权限");
                }
                ...
                if (updatedBomDetails != null && updatedBomDetails.Count() > 0)
                {
                    foreach (BomDetail bomDetail in updatedBomDetails)
                    {
                        ...
                        if (!string.IsNullOrEmpty(bomDetail.FeedLocation))
                        {
                            ...
                            Location location = base.genericMgr.FindAll<Location>(locationcheckStr, new object[] { flowMaster.PartyFrom, bomDetail.FeedLocation }).SingleOrDefault<Location>();
                            if (location == null)
                            {
                                throw new BusinessException("物料" + bomDetail.Item + "的库位不正确");
                            }
             ...
             "updated")]IEnumerable<BomDetail> updatedBomDetails)
        {
            try
            {
                ...
                FlowMaster flowMaster = base.genericMgr.FindById<FlowMaster>(productLineLocationDetail.ProductLine);
        ...
        public JsonResult BackFlush(ProductLineLocationDetail productLineLocationDetail, string itemStr, string qtyStr)
        {
            try
            {
                ...
                SaveSuccessMessage(Resources.PRD.ProductLineLocationDetail.ProductLineLocationDetail_BackFlushed);
                return Json(new { });
            }
            catch (BusinessException ex)
            {
             ...
             "updated")]IEnumerable<BomDetail> updatedBomDetails)
        {
            try
            {
                ...
                SaveSuccessMessage(Resources.PRD.ProductLineLocationDetail.ProductLineLocationDetail_ReturnOut);
                return Json(new { });
            }
            catch (BusinessException ex)
            {
        ...
        private SearchStatementModel PrepareSearchStatement(GridCommand command, ProductLineLocationDetailSearchModel searchModel)
        {
            ...
            SecurityHelper.AddFlowPermissionStatement(ref whereStatement, "p", "ProductLine");
